...
`main()`方法在运行时会生成一个针对此方法的线程。~~而在启动时会同时生成`main()`线程和自定义（Thread）线程~~，启动JVM时会先创建执行`main()`的线程，后续线程的创建由在代码中定义的位置确定，执行顺序由哪个线程先获得CPU决定。
...
获得当前线程优先级使用`getPriority():int`方法
...
- `synchronized(handle)`中的参数需要使用到本对象中的已申明的句柄或有返回值的方法或`this`。
...
- 使用`notify()`时，会**仅唤醒一个**在此对象的监视器上执行等待的线程，若有多个线程则**随机唤醒**。（即凡是执行了/执行到对应对象的synchronized方法/代码块的线程且处于wait状态）
...
- `setMaxPriority()`，设置线程组的最高优先级，子线程组的最高等级无法高于父线程组。TIPS：在构建子类线程组时，子类线程组会继承父类的线程组的可设置最高优先级。
- 新创建的线程组将自动归属与系统线程组下。
- `new Thread(ThreadGroup, Runnable)`,设置某新线程于某线程组中，新线程的优先级无法高过所在线程组的最高优先级，**若组最高级低于线程默认优先级，此时会使用默认优先级创建新线程**。此时若对新建线程调用`setPriority()`，其结果会受到组最高级影响。Tips:若使用无ThreadGroup参数的构造器，则默认将线程置于系统线程组中。
