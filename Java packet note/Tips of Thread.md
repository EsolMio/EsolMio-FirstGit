...
From "Thing in Java"
- 在指定对象上使用`wait()`, `notify()`, `notifyAll()`时，使用此方法时当前线程**必须**需拥有此**对象的监视器(Monitor)**。**Tips**：此时无论synchronized()中的锁对象句柄的名称为何，只要句柄指向的对象一致，效果则相同
...
- 使用`notify()`时，会**仅唤醒一个**在此对象的监视器上执行等待的线程，若有多个线程则随机唤醒。（即凡是线程执行了对应对象的synchronized方法/）
- 
