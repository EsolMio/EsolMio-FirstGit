...
- 若需要自定义序列化方式，有两种选择：
   ...
   - 创建两个private修饰的`private void writeObject(ObjectOutputStream stream) throws IOException`和`private void readObject(ObjectInputStream stream) throws IOException, ClassNotFoundException`方法，ObjectOutput/ObjectInput会自动先寻找
