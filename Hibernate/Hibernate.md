...
1. 实现`ConnectionProvider`接口，此接口提供了：
	...
	- Tips：在`getConnection()`中获得一个Connection对象前需要先对连接池配置再创建Connection（若使用Apache DBCP，可以在`BasicDataSource`中使用编程配置，或者使用`BasicDataSourceFactory.createDataSource(Properties pro)`通过外部）
